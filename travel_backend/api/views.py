        ...
        if search:
            for term in search.split():
                tours = tours.filter(
                    ...
                    | Q(departure_city__icontains=term)
                    | Q(region_key__icontains=term)
                    | Q(hero_label__icontains=term)
                )
        if region:
...
class PopularToursAPIView(APIView):
    ...
    def get(self, request):
        tours = (
            Tour.objects.select_related("destination", "owner")
            .filter(is_published=True, popular=True)
            .order_by("-created_at")
        )
        serializer = TourSerializer(tours, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class ContactsCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"id": inquiry.id, "message": "РЎРїР°СЃРёР±Рѕ! РњС‹ РїРѕР»СѓС‡РёР»Рё СЃРѕРѕР±С‰РµРЅРёРµ Рё СЃРєРѕСЂРѕ РѕС‚РІРµС‚РёРј."},
            status=status.HTTP_201_CREATED,
        )
